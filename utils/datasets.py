...
class LoadMaskedImages:
    ...
    def __init__(self, path, img_size=640, stride=32, mask=None, coef = 1, auto=True):
        ...
        self.mask = mask #points on original shaped image
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            self.mode = 'video'
            ret_val, img0 = self.cap.read()
            self.cap.set(cv2.CAP_PROP_POS_FRAMES, self.cap.get(cv2.CAP_PROP_POS_FRAMES) + self.step)
            while not ret_val:
                self.count += 1
                self.cap.release()
                if self.count == self.nf:  # last video
    ...
    def new_video(self, path):
        ...
        if self.coef != 0:
            self.step = int(self.cap.get(cv2.CAP_PROP_FPS)/self.coef)
        else:
            self.step = 1
